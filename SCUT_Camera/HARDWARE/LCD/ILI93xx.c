...
/*
 * 函数名：LCD_Show_Image()
 * 描述  ：将数组中的数据显示为一张图片。 2017-06-20 By BachMan
 * 输入  ：无
 * 输出  ：无
 */
...
//	u16 color_b;
//	
//	LCD_Scan_Dir(R2L_U2D);		//从左到右 ,从上到下
//	LCD_SetCursor(0x00,0x0000);	//设置光标位置 
//	LCD_Set_Window(0, 0, WIDTH, HIGHT);                // 2017-06-321 by BachMan
//	LCD_WriteRAM_Prepare();     //开始写入GRAM	
...
//		{
//			color_previw = (u16)Pic_Buff[row][col];
//			color_process = color_previw << 8;

//			//  YUYV格式的Y转化成RGB565					
//			color_r = color_process&0xf800;
//			color_b = color_process>>11;
//			color_process >>= 5;
//			color_g =color_process&0x07e0; 
...
/*
 * 函数名：LCD_Show_Image()
 * 描述  ：将数组中的数据显示为一张()图片。 2017-06-20 By BachMan
 * 输入  ：u16 x_pos, u16 y_pos  开始的位置， 应该与截图函数CAMERA_Get_ImageCut()的输入保持一致
 *				 应该确保	x_pos+width 不超320， y_pos + hight 不超过240	
 * 输出  ：无
 */
void LCD_Show_Image(u16 x_pos, u16 y_pos)      //应该确保	x_pos+width 不超320， y_pos + hight 不超过240		
{
	u16 col, row;
	u16 color_process;
	u16 color_previw;
	...
	LCD_SetCursor(x_pos,y_pos);	//设置光标位置 
	LCD_Set_Window(x_pos, y_pos, WIDTH, HEIGHT);                // 2017-06-321 by BachMan
	LCD_WriteRAM_Prepare();     //开始写入GRAM	
