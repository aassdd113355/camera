...
void cameraSysInit()
{
  ...
  OV7670_Init();
	...
	GpioInit();
	EXTIX_Init();
	TIM3_Int_Init(30000-1,8399);			//10Khz计数频率,2秒钟中断	
  TIM2_Int_Init(50000-1,16800-1);			//10Khz计数频率,10秒钟中断	
	Glass_Light_On = 1;//灯光打开
	delay_ms(40000);	
...
void Glass_Water_Level_Static(void)
{
	...
	for(i=0;i<9;i++)
	{
			if(pixel_count[i] < r_circle * r_circle * 0.03)                   //调整
			{
			pixel_count[i] = 0;
			}
		pixel_count_sort[i] = pixel_count[i];											//		静态检测									
...
//	int flag1,flag2;
	...
	for(i=0;i<9;i++)
	{
			...
			if(pixel_count[i] > r_circle * r_circle * 0.05)    //调整参数
			{
				max_level = i;
			}			
	}
